    ...
    <div class="panel-body">
        <form action="/chapter/addDo" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" value="{{$chapterInfo['chaptername']}}" name="chaptername" placeholder="添加章节名称">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail1">选择课程</label>
                <select class="form-control" id="cid" name="cid">
                    @foreach($arr as $k=>$v)
                        @if($chapterInfo['cid']==$v['cid'])
            ...
            <div class="form-group" >
                ...
                <select class="form-control" id="type" name="type">
                    <option value="1" @if($chapterInfo['type']=='章') selected @endif>章</option>
                    <option value="2" @if($chapterInfo['type']=='节') selected @endif>节</option>
                </select>
            </div>
            ...
            <div class="form-group demo" style="display: none">
                ...
                @foreach($info as $k=>$v)
                    @if($chapterInfo['pid']==$v['chapterid'])
                        <option value="{{$v['chapterid']}}" selected>{{$v['chaptername']}}</option>
                    @else
                        <option value="{{$v['chapterid']}}" >{{$v['chaptername']}}</option>
                    @endif
                @endforeach
                </select>
            </div>
